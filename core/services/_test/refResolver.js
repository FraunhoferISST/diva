...
const buildMappingArtifact = (key, type, _elasticsearch) => {
  ...
  if (_.isPlainObject(_elasticsearch) && _elasticsearch.type !== undefined) {
    console.log(chalk.cyanBright(`🐛 found ES Type Property!`));
  ...
  if (_.toLower(type) === "string") {
    if (
      ...
      console.log(chalk.cyanBright(`🐛 found ES Analyzer Property!`));
...
const propertyIsScalar = (pT) => {
  if (
    ...
    console.log(chalk.blue(`🐛 "propertyIsScalar": found scalar type ${pT}`));
    return true;
  }
...
const propertyIsArray = (pT) => {
  if (_.isString(pT) && _.toLower(pT) === "array") {
    console.log(chalk.blue(`🐛 "propertyIsArray": found array type`));
    return true;
  }
...
const propertyIsObject = (pT) => {
  if (_.isString(pT) && _.toLower(pT) === "object") {
    console.log(chalk.blue(`🐛 "propertyIsObject": found object type`));
    return true;
  }
...
const buildMapping = (schema) => {
  ...
  for (const [key, value] of Object.entries(schema)) {
    if (combinationKeys.includes(key)) {
      console.log(chalk.green(`🐛 need to iterate array...`));
      schema[key].forEach((e) => {
        console.log(chalk.green(`🐛 recursive call`));
      ...
      for (const [pk, pv] of Object.entries(value)) {
        ...
        if (hasCombinations(pv)) {
          ...
          for (const elem of getCombinationsKeys(pv)) {
            console.log(chalk.green(`🐛 recursive call`));
            mapping = { ...mapping, ...buildMapping(elem) };
          }
          mappingElements.push({ [pk]: { properties: mapping } });
        }
        ...
        if (propertyIsObject(pv.type)) {
          ...
          mappingElements.push({ [pk]: { properties: objMapping } });
        }
      }
    ...
    if (key === "then") {
      tmp = { ...tmp, ...buildMapping(schema.then) };
    }
  }
  return tmp;
};
