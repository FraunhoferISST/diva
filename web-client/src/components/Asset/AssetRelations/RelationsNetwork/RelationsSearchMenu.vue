  ...
  <v-menu v-model="menu" max-width="500px" :close-on-content-click="false">
    ...
    <v-card>
          ...
          <v-row>
                ...
                <template #item="{ item }">
                  <template>
                    ...
                    <v-list-item-content>
                      <v-list-item-title>
                        <b>
                          {{ item.doc.title || item.doc.username }}
                        </b>
                      </v-list-item-title>
                      <v-list-item-subtitle
                        v-html="item.doc.entityType"
                      ></v-list-item-subtitle>
                    </v-list-item-content>
                  </template>
                </template>
            ...
            <v-col cols="12">
              <div class="relations-search-selected-items pa-3">
                <div>
                  <fade-in
                    v-for="selectedItem in selectedEntities"
                    :key="selectedItem.doc.id"
                  >
                    <relations-search-entity-card :entity="selectedItem.doc" />
                  </fade-in>
                </div>
              </div>
            </v-col>
      ...
      <v-card-actions>
        ...
        <v-btn text color="error" small rounded @click="close">Cancel</v-btn>
        <v-btn
          color="primary"
          rounded
          small
...
export default {
  ...
  components: { RelationsSearchEntityCard, FadeIn },
  setup() {
  ...
  computed: {
    ...
    foundedEntities() {
      return this.searchResult.filter(({ doc }) =>
        this.selected.every((selected) => doc.id !== selected.doc.id)
      );
    },
    searchResultFormatter() {
      return this.searchResult.filter(({ doc }) => doc.entityType !== "review");
    },
  },
  methods: {
    close() {
